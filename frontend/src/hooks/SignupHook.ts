...
export interface SignUpForm {
    ...
    emailLocal: string;     // 일반 모드: @ 앞
    emailDomain: string;    // 일반 모드: @ 뒤 (select 값)
    customEmail?: string;   // 커스텀 모드: 전체 이메일
    phone?: string;
    address?: string;
...
export function SignupHook() {
    ...
    const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        // 일반 모드: emailLocal 검사
        if (!isCustomEmail && name === "emailLocal") {
        ...
        // 최종 이메일 유효성 검사
        if (
            (isCustomEmail && name === "customEmail") ||
            (!isCustomEmail && (name === "emailLocal" || name === "emailDomain"))
        ) {
            const fullEmail = isCustomEmail
                ? form.customEmail || value
